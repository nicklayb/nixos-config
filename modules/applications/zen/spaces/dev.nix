{ builder }: let
  ...
  elmFolder = { title = "Elm"; uuid = "186c2406-2dab-41e4-8bc6-c62434bfef44"; };

  elmlang = sites: builder.multiPins {
    title = "Elm";
    baseUrl = path : "https://package.elm-lang.org/packages/${path}";
    folderId = elmFolder.uuid;
    sites = map (item : item // { title = item.url; }) sites;
  };
...
in {
  ...
  icon = "fingerprint";
  id = 4;
  folders = [hexdocFolder elmFolder];
  theme = [
