...
let
  ...
  containerToPins =
    ...
    acc ++ map (itemToPin container) pins;
  ...
  putFolder =
    ...
    // {
      title = title;
      folderId = folderId;
      url = (if isNull baseUrl then site.url else baseUrl site.url);
    };
  mkFolder =
    folder:
    let
      folderId = uuid folder.title;
      folderItem = itemToFolder folder;
    in
    [ (folderItem // { uuid = folderId; }) ] ++ map (putFolder folderId folder) folder.sites;
  mkPin =
    {
      url,
      title ? url,
