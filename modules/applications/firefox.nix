{ config, lib, mainUser, username, pkgs, inputs, ... }: {
  ...
  config = lib.mkIf config.mods.firefox.enable {
    ...
    home-manager.users.${username} = {
      home.file.".mozilla/firefox/${username}/chrome/firefox-mod-blur".source = inputs.firefox-firefox-mod-blur;
      home.file.".mozilla/firefox/${username}/chrome/textfox".source = inputs.firefox-textfox;
      programs.firefox = {
        enable = true;
        profiles.${username} =
          let
